...
def test_routes_exact(): assert routes()==(("aerodrome","uniswap_v3"),("uniswap_v3","aerodrome"))
def test_rejects_bad_routes():
 with pytest.raises(ValueError): validate_two_leg_route("sushi","aerodrome","a","b")
...
def test_profit_and_fee():
 c=CostModel(Decimal(".10"),Decimal(".02"),Decimal(".01")); assert net_profit(Decimal("100"),Decimal("101"),c)==Decimal(".87"); assert flash_loan_fee(Decimal("100"),5)==Decimal(".05")
def test_base_lock(): assert Config.from_env().chain_id==8453
